...
//float compensation2[TICKS_PR_REV] = {0,0.5,-0.5,0,0,0,0,-0.8,0,-0.5,0.5,0,2,-2,0};
//float compensation2[TICKS_PR_REV] = {0,0.8,-0.7,1,1,0,0,-0.8,-0.6,-0.5,0.5,0,2,-2,0};
...
- (id) initWithDirDelegate:(id<VaavudElectronicWindDelegate>)delegate {
    ...
    for (int i = 0; i < TICKS_PR_REV; i++) {
        compensation[i] = compensation[i]*(compensation2[i]/100+1);
    }
    
    // reverse fit curve
    
    float fitcurveReversed[360];
    
    for (int i = 0; i < 360; i++) {
        fitcurveReversed[i] = fitcurve[359-i];
    }
...
- (void) newTick:(int)samples {
    ...
    if (tickCounter == TICKS_PR_REV-1) {
        ...
        if (samples < 0.6 * lastSample || samples > 1.9 * lastSample) {
